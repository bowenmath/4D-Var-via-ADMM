#---------------------------------------------------------------
# This program draws the X-axis slices of the objective function.
#---------------------------------------------------------------
# By Bowen Li
# October 7, 2024
#---------------------------------------------------------------
...
H = 500 # resolution, number of points
n = 3 # number of surfaces
yy = np.linspace(-5,5,H)
zz = np.linspace(15,25,H)
...
for k in range(n):
    for i in range(H):
        for j in range(H):
            X[k, i, j] = -5 + 5*k
            obj_f[k, i, j] = obj([X[k, i, j], Y[i,j], Z[i,j]])
...
for k in range(n):
   ...
   surf = ax.plot_surface(X[k], Y, Z, facecolors=plt.cm.viridis((obj_f[k] - min_f)/(max_f-min_f)), rstride=1, cstride=1, shade=False)
...
plt.savefig('X.png', dpi=600, format='png', bbox_inches='tight')  # Use high DPI when saving, then use an image tool to convert the PNG to PDF to reduce the size of figure
