#---------------------------------------------------------------
# This program draws the Y-axis slices of the objective function.
#---------------------------------------------------------------
# By Bowen Li
# October 7, 2024
#---------------------------------------------------------------
...
for k in range(n):
    for i in range(H):
        for j in range(H):
            obj_f[k, i, j] = obj([X[i,j], -5 + 5*k, Z[i,j]])
            Y[k, i, j] = -5 + 5*k
...
for k in range(n):
   ...
   surf = ax.plot_surface(X, Y[k], Z, facecolors=plt.cm.viridis((obj_f[k] - min_f)/(max_f-min_f)), rstride=1, cstride=1, shade=False)
...
plt.savefig('Y.png', dpi=600, format='png', bbox_inches='tight')  # Use high DPI when saving, then use an image tool to convert the PNG to PDF to reduce the size of figure
