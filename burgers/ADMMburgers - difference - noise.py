...
# Burgers equation with finitie difference discretization.
#---------------------------------------------------------
# By Bowen Li
# October 7, 2024
#---------------------------------------------------------
...
def f(x, dt):
   ...
   """brugers model with finite difference method"""
   # Setting up vector
   d = np.zeros(N)
...
dt = 0.02  # The size of time steps
num_steps = 100  # The number of time steps
...
plt.savefig('convergence_FD.pdf', dpi=60, format='pdf', bbox_inches='tight')
...
plt.savefig('cons_err_FD.pdf', dpi=60, format='pdf', bbox_inches='tight')
...
for i in range(4):
    axs[i].scatter(xx, xs[i*int(0.6/dt)], label='FD-Obs', c='magenta', marker='o', s=12)
    axs[i].plot(xx, xst[i*int(0.6/dt)], label='FD-True', color='#1f77b4', linewidth=3)
    ...
    axs[i].legend([handles[0], handles[1], handles[2]], [labels[0], labels[1], labels[2]], fontsize=10, loc='upper left', frameon=True, framealpha=0.8, edgecolor='black')  # 显示图例
...
plt.savefig('dynamic_FD.pdf', dpi=60, format='pdf', bbox_inches='tight')
